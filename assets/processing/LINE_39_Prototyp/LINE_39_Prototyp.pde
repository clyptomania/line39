...
//Distance Threashold
int maximaleDistanz = 3500; // 4500 max 4.5mx
...
void setup() {
  size(512, 424, P3D);
  opencv = new OpenCV( this, Trackingbreite, Trackinghoehe);
...
void draw() {
  ...
  ArrayList<Float> PersonXSkaliertList = new ArrayList();
  ...
  for (Contour contour : contours) {
    ...
    PersonXSkaliert = (((PersonX / Trackingbreite)*0.87)+0.06);
    println("show me PersonXSkaliert", PersonXSkaliert);
    PersonYSkaliert = PersonY / Trackinghoehe;
    ...
    if (Person.width > Pixelgroesse) {
      PersonXSkaliertList.add(PersonXSkaliert);
  ...
  for (int i=0; i<PersonXSkaliertList.size(); i++) {
    message += "," + str(PersonXSkaliertList.get(i));
